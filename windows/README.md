...
- **App icon:** `ime/res/viettelex.ico` holds 16, 20, 24, 32, 40, 48, 64 and 256 px. Sizes that exist as `AppIcon.appiconset` PNGs are copied byte for byte; the rest are `sips` downscales. It is used for the exe, the MSI's Add/Remove entry, the Start menu shortcut, the settings title bar and the TSF profile.
- **Tray and input-indicator glyphs:** `ime/res/glyph_<vt|star|flag>_<v|e>_<dark|light>.ico`. They are rasterised from `MenuIcon1/2/3.pdf`, the same mapping as `MenuIconSwitcher.swift`:
  - `dark` is a white glyph for the dark taskbar; `light` is a near-black glyph for the light taskbar, chosen from `SystemUsesLightTheme`.
  - `e`, the English state, is the same glyph at 40% opacity, because macOS has no English variant of its own glyph.
