...
## Per-host text policy (1.1.1)

In-place is the default. It needs a TSF document that really contains the text before the caret, and some hosts don't have one:

- **Consoles** (cmd, PowerShell, WSL windows in conhost/OpenConsole, Windows Terminal):
  - TSF is activated with `TF_TMAE_CONSOLE`, and Windows Terminal's context is `TS_SS_TRANSITORY`.
  - The document is only the composition. Text outside it goes straight to the shell and can't be read or deleted, which is how 1.1.0 typed "thuưr gox" in cmd.
  - Policy: composition only, with no re-edit and no ⌫ re-open.
  - Built-in app rules cover conhost, openconsole, windowsterminal and mintty (Git Bash, which uses IMM only).
- **Read-only** (`TF_SD_READONLY`): keys pass through.
- **Unrecognised hosts whose document shows none of the typed text** (xterm.js in the VS Code terminal, other IMM-only apps): at the second key the session switches the field to composition.
- **Search bars** (Start/Windows Search, Explorer, Settings, Chrome/Edge omnibox and page fields, Office, PowerToys Run, Everything) are ordinary readable TSF/CUAS stores and use verified in-place:
  - a forward autocomplete selection belongs to the app;
  - a box that re-sets its own text keeps our word;
  - anything that doesn't verify falls back to composition for that field.
