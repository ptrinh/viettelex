// VietTelex Core — JavaScript SDK (shared implementation).
//
// Entry points (package.json "exports") bind this to one WebAssembly build:
//   @viettelex/core                 full engine, WASM inlined (one file)
//   @viettelex/core/lite            no English dictionaries, WASM inlined
//   @viettelex/core/external        full engine, separate viettelex-core.wasm (cacheable)
//   @viettelex/core/lite/external   lite engine, separate viettelex-lite-core.wasm
//
...
/**
 * Flags that need the English dictionaries. The lite build has no dictionaries, so these
 * are always cleared there (the options are accepted and silently ignored).
 */
export const ENGLISH_TABLE_FLAGS = Flags.contextualEnglish | Flags.englishWordRestore | Flags.collisionPrefersVietnamese;
...
export class VietTelexEngine {
  ...
  constructor(module, options = {}) {
    this.#m = module;
    /** true for the lite build (no English dictionaries). */
    this.lite = !!module.vtxLite;
  ...
  setOptions(options = {}) {
    ...
    this.#setFlags(flagsFromOptions(this.options));
  }
  #setFlags(f) {
    if (this.lite) f &= ~ENGLISH_TABLE_FLAGS;
    this.#m._vtx_set_flags(this.#e, f >>> 0);
  }
...
/**
 * Bind the SDK to one emscripten module factory. Returns { loadModule, createEngine, attach }.
 * `loadModule({ wasmUrl, wasmBinary })`: only the external builds read these (where to fetch the
 * .wasm; default = next to the JS file). The module is loaded once per build.
 */
export function bindWasm(factory, { lite = false } = {}) {
  let modulePromise = null;
  function loadModule(opts = {}) {
    return (modulePromise ??= (async () => {
      const cfg = {};
      if (opts.wasmUrl) cfg.locateFile = () => String(opts.wasmUrl);
  ...
  async function createEngine(options = {}) {
    return new VietTelexEngine(await loadModule(options), options);
  }
  return { loadModule, createEngine, attach: (el, options = {}) => attachWith(createEngine, el, options) };
}
 ...
 */
async function attachWith(createEngine, el, options) {
